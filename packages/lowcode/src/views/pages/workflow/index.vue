...
<template>
  <div class="workflow-container layout-padding">
    <div v-if="state.isShow" class="workflow-mask" />
    <div class="layout-padding-auto layout-padding-view workflow-warp">
      <div class="workflow">
        ...
        <div class="workflow-content">
            ...
            <el-scrollbar>
              ...
              >
                <div class="workflow-left-title" @click="onTitleClick(val)">
                  <span>{{ val.title }}</span>
                  <SvgIcon
                    :name="val.isOpen ? 'ele-ArrowDown' : 'ele-ArrowRight'"
                  />
                </div>
                <div
          ...
          <!-- 右侧绘画区 -->
          <div ref="workflowRightRef" class="workflow-right">
            <div
            ...
            >
              <div
                ...
                :class="{
                  'workflow-right-active': state.jsPlumbNodeIndex === k,
                }"
              >
                <div class="workflow-left-item-icon">
                  <SvgIcon :name="v.icon" class="workflow-icon-drag" />
    ...
    Sortable.create(v as HTMLDivElement, {
      ...
      onEnd(evt: any) {
        ...
        if (
          ...
          // 节点id（唯一）
          const nodeId = Math.random().toString(36).slice(2, 14)
          // 处理节点数据
          const node = {
            nodeId,
            left: `${layerX - 40}px`,
...
const onContextmenu = (v: any, k: number, e: MouseEvent) => {
  ...
  state.leftNavList.forEach((l) => {
    if (l.children)
      if (l.children.some((c: any) => c.id === v.id))
...
const onToolDownload = () => {
  ...
  const href = `data:text/json;charset=utf-8,${encodeURIComponent(
    JSON.stringify(state.jsplumbData, null, '\t')
  )}`
